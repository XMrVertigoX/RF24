...
template <typename TYPE> inline TYPE LEFT(TYPE byte, char amount) {
  return (byte << amount);
}

template <typename TYPE> inline void LEFT_eq(TYPE &byte, char amount) {
  byte <<= amount;
}

template <typename TYPE> inline TYPE RIGHT(TYPE byte, char amount) {
  return (byte >> amount);
}

template <typename TYPE> inline void RIGHT_eq(TYPE &byte, char amount) {
  byte >>= amount;
}

template <typename TYPE> inline TYPE INVERT(TYPE byte) { return (compl(byte)); }
